"""Exported model wrapper satisfying the Evaluatable protocol.

Loads models from the HF-style export directory produced by
src/training/export.py (model.safetensors + config.json + tokenizer.json).
"""
...
from src.config.base import ExperimentConfig, ModelConfig, DataConfig
from src.data.tokenizer import CharTokenizer, HFTokenizer, build_tokenizer
from src.training.run import _build_model
from src.utils.device import resolve_device


class ExportedModel:
    """Wraps a model exported to a directory for evaluation.
    ...
    Usage:
        model = ExportedModel("models/my-run")
    ...
    def __init__(self, model_dir: str):
        ...
        vocab_size = model_config.pop("vocab_size")
        max_seq_len = model_config.pop("max_seq_len")
        ...
        if "chars" in tok_data:
            self._tokenizer = CharTokenizer(tok_data["chars"])
        elif tok_data.get("type") == "hf":
            self._tokenizer = build_tokenizer(tok_data["name"])
        else:
        ...
        experiment_config = ExperimentConfig(
            name="eval",
            model=ModelConfig(name=model_type, extra_args=model_config),
            data=DataConfig(max_seq_len=max_seq_len),
        )
        self._model = _build_model(experiment_config, vocab_size).to(self._device)

        # Load weights
        load_model(self._model, os.path.join(model_dir, "model.safetensors"))
        self._model.eval()
        self._max_seq_len = max_seq_len

        param_count = sum(p.numel() for p in self._model.parameters())
        print(f"Loaded exported model from {model_dir} ({param_count / 1e6:.1f}M params) on {self._device}")
