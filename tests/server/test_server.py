...
from src.storage import Database, EvalDatabase
...
def client(db_path, seeded_db, eval_db):
    ...
    import src.server.state as state
    import src.server.routes.runs as runs_mod
    import src.server.routes.evals as evals_mod

    old_db = state.run_manager.db
    old_eval_db = state.eval_db

    # Patch run_manager.db
    state.run_manager.db = seeded_db[0]

    # Patch eval_db everywhere it was imported at module level
    state.eval_db = eval_db
    server_app.eval_db = eval_db
    runs_mod.eval_db = eval_db
    evals_mod.eval_db = eval_db

    yield TestClient(server_app.app)

    # Restore
    state.run_manager.db = old_db
    state.eval_db = old_eval_db
    server_app.eval_db = old_eval_db
    runs_mod.eval_db = old_eval_db
    evals_mod.eval_db = old_eval_db
...
class TestGetTrainingState:
    def test_returns_full_state(self, client, seeded_db):
        ...
        series = state["series"]
        assert "trainLoss" in series
        assert "lr" in series
        assert "gradNorm" in series
        assert "bpc" in series

        # trainLoss series should have 3 data points (steps 0, 10, 20)
        assert len(series["trainLoss"]) == 3
        assert series["trainLoss"][0]["step"] == 0
        assert series["trainLoss"][0]["value"] == 4.2
        assert series["lr"][0]["value"] == 1e-5
        assert series["gradNorm"][0]["value"] == 1.5

        # valLoss series should have 1 data point (step 10)
        assert "valLoss" in series
        assert series["valLoss"][0]["step"] == 10
        assert series["valLoss"][0]["value"] == 4.0
...
class TestStopRun:
    def test_stop_no_active_run(self, client):
        resp = client.post("/api/runs/999/stop")
