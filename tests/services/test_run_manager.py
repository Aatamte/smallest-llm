...
class TestStop:
    def test_stop_when_no_run(self, manager):
        run_id = manager.db.create_run("no-active", {})
    ...
    def test_stop_sets_should_stop_and_waits(self, manager):
        ...
        ok = manager.stop(run_id, timeout=5.0)
        assert ok is True
    ...
    def test_stop_timeout_marks_failed(self, manager):
        ...
        # Thread that ignores should_stop but can be interrupted
        def stuck_run():
            stop_event.wait(10)
        ...
        ok = manager.stop(run_id, timeout=0.1)
        assert ok is False

        run = manager.db.get_run(run_id)
        assert run["status"] == "failed"

        # Cleanup: release the stuck thread
        stop_event.set()
...
class TestStopRaceCondition:
    ...
    def test_stop_flag_prevents_thread_from_setting_completed(self, manager):
        ...
        def fake_run():
            ...
            if not manager._stop_requested.is_set():
