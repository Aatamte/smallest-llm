...
class TestEvals:
    ...
    def test_log_and_get_eval(self, db, eval_db):
        run_id = db.create_run("test", {})
        eval_db.log_eval(
            task="perplexity",
    ...
    def test_standalone_eval_no_run(self, eval_db):
        eval_db.log_eval(
            task="blimp",
            metrics={"accuracy": 0.82},
            model_name="smollm-135m",
        )
        evals = eval_db.get_evals()
        assert len(evals) == 1
        assert evals[0]["run_id"] is None
        assert evals[0]["model_name"] == "smollm-135m"

    def test_filter_by_task(self, eval_db):
        eval_db.log_eval(task="perplexity", metrics={"perplexity": 45.0})
        eval_db.log_eval(task="blimp", metrics={"accuracy": 0.8})
        eval_db.log_eval(task="perplexity", metrics={"perplexity": 40.0})

        ppl_evals = eval_db.get_evals(task="perplexity")
        assert len(ppl_evals) == 2

        blimp_evals = eval_db.get_evals(task="blimp")
        assert len(blimp_evals) == 1

    def test_filter_by_run_and_task(self, db, eval_db):
        r1 = db.create_run("run1", {})
        r2 = db.create_run("run2", {})
        eval_db.log_eval(task="perplexity", metrics={"ppl": 50}, run_id=r1)
        eval_db.log_eval(task="perplexity", metrics={"ppl": 40}, run_id=r2)

        evals = eval_db.get_evals(run_id=r1, task="perplexity")
        assert len(evals) == 1
